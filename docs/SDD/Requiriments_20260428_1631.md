...
**Gerado:** 2026-04-28 | **Revisão:** v3 — adição NFR-03 (containerização Docker/Linux)  
**Fonte da verdade:** este documento prevalece sobre Requiriments_20260428_1614.md
...
* **NFR-03 (Containerização e Portabilidade Linux):** Os extratores Python devem ser executáveis em container Docker (Linux) sem modificações de código. A camada VBA/Excel é o único componente não-portável e será substituída na migração para banco de dados (NFR-01). Dependências de sistema operacional (bibliotecas nativas, variáveis de ambiente, caminhos de arquivo) devem ser isoladas em configuração de ambiente — nunca hardcoded nos scripts. O Dockerfile deve ser o artefato de deployment de produção após validação do MVP.
