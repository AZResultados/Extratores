...
| TASK-01 Fail-Fast | PENDENTE |
| TASK-02 CLI+Envelope | PENDENTE |
...
pdfplumber      0.11.9
pikepdf         10.5.1  (instalado, NÃO integrado — etapa manual)
...
└── docs/
    ├── Requiriments.md
    ├── Design_Doc_20260428_0350.md
    ├── Tasks_20260428_0421.md
...
[botão Excel] → ModComum.ProcessarExtrator()
  └─ WScript.Shell.Exec()
     ...
     → Python: json.dumps(envelope) → sys.stdout  [zero disco]
...
| Mercado Pago | "ND" |
| Uso atual | MANUAL — operador lê e aplica pikepdf fora do script |
| Integração | NENHUMA — scripts recebem PDFs já descriptografados |
...
### TASK-02 — CLI + Envelope JSON BR-02+BR-04
Arquivos: src/cartao_mercadopago.py + src/cartao_santander.py
...
Cada lançamento deve ter: cliente, id_lote, arquivo (path.name), vencimento, descricao, parcela, valor, tipo, titular_cartao.
...
### TASK-03 — pikepdf in-memory BR-06
...
if args.password:
    with pikepdf.open(pdf_path, password=args.password) as pdf:
        buf = io.BytesIO()
        pdf.save(buf)
        buf.seek(0)
...
' ExitCode = 0 com stderr não-vazio → aviso técnico (não abortar)
...
| ID | Decisão |
|----|---------|
| A1 | pikepdf in-memory via io.BytesIO |
...
gerado: 2026-04-28 | fonte: Design_Doc_0350 + Tasks_0421 + SnapShot_0100 + Checkpoint_0250
